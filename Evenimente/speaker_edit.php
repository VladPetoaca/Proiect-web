...
{ if (isset($_POST['submit']))
    ...
    if (is_numeric($_POST['id']))
    {
        ...
        {
            if ($stmt = $mysqli->prepare("UPDATE speakeri SET 
                       nume = ?,email=?,telefon=?,adresa=? WHERE id='".$id."'"))
            {
                $stmt->bind_param("ssss", $nume, $email, $telefon, $adresa);
                $stmt->execute();
                $stmt->close();
                echo "Datele despre speaker au fost actualizate cu succes!";
            }
            else
            {echo "Nu s-au putut modifica datele despre speaker.";}
        }
    }
...
<html>
<head>
    <title> <?php if ($_GET['id'] != '') { echo "Actualizare Speaker"; }?> </title>
    <meta http-equiv="Content-Type" content="text/html; charset=utf8"/>
    <link rel="stylesheet" href="crud.css">
</head>

<body>
<h1><?php if ($_GET['id'] != '') { echo "Actualizare Speaker"; }?></h1>
<?php if ($error != '') {
